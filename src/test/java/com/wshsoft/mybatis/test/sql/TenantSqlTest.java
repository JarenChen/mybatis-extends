...
public class TenantSqlTest {
	...
	public void intsertInto() {
		...
		Assert.assertEquals(
				"INSERT INTO role (c1, c2, c3, tenant_id) SELECT *, tenant_id FROM bak WHERE tenant_id = 1000",
				sqlInfo.getSql());
	}
	...
	@Test(expected = MybatisExtendsException.class)
	...
	public void deleteOrderBy() {
		...
		Assert.assertEquals("DELETE FROM tablename WHERE tenant_id = 1000 AND a = 1 AND b = 1 ORDER BY col",
				sqlInfo.getSql());
	}

	@Test
	public void deleteJoin() {
		SqlInfo sqlInfo = tenantSqlParser.optimizeSql(null,
				"DELETE gc FROM guide_category AS gc LEFT JOIN guide AS g ON g.id_guide = gc.id_guide WHERE g.title IS NULL LIMIT 5");
		Assert.assertEquals(
				"DELETE gc FROM guide_category AS gc LEFT JOIN guide AS g ON g.id_guide = gc.id_guide WHERE gc.tenant_id = 1000 LIMIT 5",
				sqlInfo.getSql());
	}
	...
	public void updateSet() {
		...
		Assert.assertEquals("UPDATE role SET c1 = 'as', c2 = ?, c3 = 565 WHERE tenant_id = 1000 AND o >= 3",
				sqlInfo.getSql());
	}
	...
	public void updateSetJoin() {
		SqlInfo sqlInfo = tenantSqlParser.optimizeSql(null,
				"UPDATE role SET c1 = 5 FROM role LEFT JOIN user ON c1 = c2 Where o >= 3");
		Assert.assertEquals(
				"UPDATE role SET c1 = 5 FROM role LEFT JOIN user ON c1 = c2 WHERE tenant_id = 1000 AND o >= 3",
				sqlInfo.getSql());
	}
	...
	public void selectChild() {
		...
		Assert.assertEquals("SELECT aaa, bbb, (SELECT ccc FROM user) AS ccc FROM role WHERE tenant_id = 1000",
				sqlInfo.getSql());
	}
	...
	public void selectChild1() {
		SqlInfo sqlInfo = tenantSqlParser.optimizeSql(null,
				"select name from role where  id= (select rid from user where id=1 )");
		Assert.assertEquals("SELECT name FROM role WHERE tenant_id = 1000 AND id = (SELECT rid FROM user WHERE id = 1)",
				sqlInfo.getSql());
	}
	...
	public void selectJoin() {
		SqlInfo sqlInfo = tenantSqlParser.optimizeSql(null,
				"SELECT u.aaa, r.bbb FROM role r left JOIN user u WHERE r.id=u.id");
		Assert.assertEquals("SELECT u.aaa, r.bbb FROM role r LEFT JOIN user u WHERE r.tenant_id = 1000 AND r.id = u.id",
				sqlInfo.getSql());
	}
	...
	public void selectJoin1() {
		SqlInfo sqlInfo = tenantSqlParser.optimizeSql(null,
				"SELECT u.aaa, r.bbb FROM role r LEFT JOIN user u ON r.id=u.id LEFT JOIN teacher t ON r.id=t.id ");
		Assert.assertEquals(
				"SELECT u.aaa, r.bbb FROM role r LEFT JOIN user u ON r.id = u.id LEFT JOIN teacher t ON t.tenant_id = 1000 AND r.id = t.id WHERE r.tenant_id = 1000",
				sqlInfo.getSql());
	}
}
