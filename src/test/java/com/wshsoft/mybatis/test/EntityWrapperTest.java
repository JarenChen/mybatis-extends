...
public class EntityWrapperTest {
    ...
    public void testQbc() {
        ...
        String sqlPart = Condition.instance().gt("gt", 1).le("le", 2).lt("le", 3).ge("ge", 4).eq("eq", 5).allEq(map)
                .toString();
        System.out.println("sql ==> " + sqlPart);
        Assert.assertEquals(
    ...
    public void testlike() {
        String sqlPart = Condition.instance().like("default", "default", SqlLike.DEFAULT)
    ...
    public void testIsWhere() {
         ...
         */
        ew.setEntity(new User(1));
        String sqlPart = ew.or("sql = {0}", "sql").like("default", "default", SqlLike.DEFAULT)
                .like("left", "left", SqlLike.LEFT).like("right", "right", SqlLike.RIGHT).isWhere(false)
                .eq("bool", true).toString();
        System.out.println("sql ==> " + sqlPart);
        Assert.assertEquals("WHERE (default LIKE '%default%' AND left LIKE '%left' AND right LIKE 'right%')", sqlPart);
    }
}
