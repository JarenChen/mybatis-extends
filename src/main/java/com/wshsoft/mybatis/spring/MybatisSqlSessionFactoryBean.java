...
public class MybatisSqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean, ApplicationListener<ApplicationEvent> {
	 ...
	 * the {@code SqlSessionFactory} configuration at runtime. This is an
	 * alternative to specifying "&lt;sqlmapper&gt;" entries in an MyBatis
	 * config file. This property being based on Spring's resource abstraction
	 * also allows for specifying resource patterns here: e.g.
	 ...
	 * DataSource for both. A transactional JDBC {@code Connection} for this
	 * {@code DataSource} will be provided to application code accessing this
	 * {@code DataSource} directly via {@code DataSourceUtils} or
	 * {@code DataSourceTransactionManager}. The {@code DataSource} specified
	 * here should be the target {@code DataSource} to manage transactions for,
	 * not a {@code TransactionAwareDataSourceProxy}. Only data access code may
	 * work with {@code TransactionAwareDataSourceProxy}, while the transaction
	 ...
	 * {@code SqlSessionFactory}. This is mainly meant for testing so that mock
	 * SqlSessionFactory classes can be injected. By default,
	 * {@code SqlSessionFactoryBuilder} creates {@code DefaultSqlSessionFactory}
	 * instances.
	 */
	 ...
	 * {@code SpringManagedTransactionFactory} The default
	 * {@code SpringManagedTransactionFactory} should be appropriate for all
	 ...
	 * statements non-transactionally. <b>It is strongly recommended to use the
	 * default {@code TransactionFactory}.</b> If not used, any attempt at
	 * getting an SqlSession through Spring's MyBatis framework will throw an
	 ...
	 * uses the standard MyBatis {@code XMLConfigBuilder} API to build a
	 * {@code SqlSessionFactory} instance based on an Reader. Since 1.3.0, it
	 * can be specified a {@link Configuration} instance directly(without config
	 * file).
	 * 
