...
public class OptimisticLockerInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (param instanceof MapperMethod.ParamMap) {
			...
			Wrapper ew = null;
			if (map.containsKey(NAME_ENTITY_WRAPPER)) {// mapper.update(updEntity,
														// EntityWrapper<>(whereEntity);
				ew = (Wrapper) map.get(NAME_ENTITY_WRAPPER);
			} // else updateById(entity) -->> change updateById(entity) to
				// updateById(@Param("et") entity)

			// TODO 待验证逻辑
			// if mannual sql or updagteById(entity),unsupport OCC,proceed as
			// usual unless use updateById(@Param("et") entity)
			...
			if (ew != null) {
				...
				if (PARAM_UPDATE_METHOD_NAME.equals(updateMethodName)) {// update(entity,
																		// null)
																		// -->>
																		// update
																		// all.
